...
def createTile(minfo, offsetX, offsetY, width, height, tilename, OGRDS):
    ...
    if MemDriver is None:
        t_fh = Driver.Create(tilename, int(width), int(height), int(bands), bt, CreateOptions)
    else:
        t_fh = MemDriver.Create(tilename, int(width), int(height), int(bands), bt)
...
def gridding(args):
    ...
    Names, TileWidth, TileHeight, Overlap, TargetDir = args
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i','--input',
                        help = 'Input tif file or list of files for processing',
                        required = True)

    parser.add_argument('-s','--size',type = int,
                        help = 'Grid size of image to be used. [Default] = 200',
                        default = 200,
                        required = True)
    ...
    inputs = argv.input
    grid_size = argv.size
    targetDir = argv.output
    
    # Taking overlap to be 10%
    overlap = grid_size * 0.1
    args = [inputs, int(grid_size), int(grid_size), int(overlap), targetDir]
    print('Gridding ...')
    try:
        gridding(args)
    except:
