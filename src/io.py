        ...
        for file in os.listdir(image_lo):
            if file.endswith(".tif"):
                if os.path.isfile(os.path.abspath(os.path.join(image_lo,file))) == False:
                    print('File %s not found'%(os.path.abspath(os.path.join(image_lo,file))))
                    continue
#                    sys.exit('File %s not found'%(os.path.abspath(os.path.join(image_lo,file))))
                ...
                if os.path.isfile(os.path.abspath(os.path.join(label_lo,file))) == False:
                    print('File %s not found'%(os.path.abspath(os.path.join(label_lo,file))))
                    continue                    
#                    sys.exit('File %s not found'%(os.path.abspath(os.path.join(label_lo,file))))
                    
                self.image.append(cv2.resize(cv2.imread(os.path.abspath(os.path.join(image_lo,file))),(image_size,image_size)))
                self.label.append(cv2.resize(cv2.imread(os.path.abspath(os.path.join(label_lo,file)),0),(image_size,image_size)))                
                count = count +1
                if count > 50000:
                    break
...
def checkres(path,size,output):
#    inputs = []
    grid_size = size
    
    # 10% overlap to be taken
    overlap = int(grid_size * 0.30) 
